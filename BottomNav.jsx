...
/* صف منتجات أفقي بأسلوب بلينكيت:
   ...
   - بطاقة «عرض المنتجات ←» في نهاية الصف تحمل مصغّرات المنتجات */
  ...
  return (
    <>
      ...
      <div className="bk-hs hide-sb">
        ...
        {onSeeAll && (
          <div className="bk-seeall-card" onClick={onSeeAll}>
            <div className="thumbs">
              {items.slice(0, 4).map((p) => (
                <span key={p.id} className="th" style={{ background: p.bg }}>{p.img ? <img src={p.img} alt="" /> : p.e}</span>
              ))}
            </div>
            <div className="arr"><ChevronLeft size={22} strokeWidth={2.8} /></div>
            <div className="lbl">عرض المنتجات</div>
          </div>
        )}
      </div>
    </>
